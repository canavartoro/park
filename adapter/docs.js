...
DocPanel = Ext.extend(Ext.Panel, {
    ...
    initComponent : function(){
        ...
        var icn = Docs.icons[this.cclass];
        if( icn == undefined ){
            icn = 'icon-cls';
        }
      
        //var ps = this.cclass.split('.');
        this.title = this.ctitle;//ps[ps.length-1];
        this.iconCls = icn;
        Ext.apply(this,{
            tbar: [/*'->',{
                text: 'Config Options',
                ...
                text: 'Sýkayet ve Oneri',
                handler: function(){
                    
                },
                iconCls: 'icon-event'
            },*/'->',{
                tooltip:'Bu modülü favori listesine ekle',
                text: 'Favorilerime Ekle',
                handler: this.directLink,
                scope: this,
                iconCls: 'icon-fav'
            }, /*'-',{
                tooltip:'Guvenli cikis',
                text: 'Guvenli Cikis',
                iconCls: 'icon-hide-inherited',
                enableToggle: true,
                scope: this,
                toggleHandler : function(b, pressed){
                     window.location = '../pages/login.jsp?go=1';
                }
            }, '-', {
...
MainPanel = function(){
    ...
    MainPanel.superclass.constructor.call(this, {
        ...
        items: {
            ...
            autoLoad:
                {
                url: 'pages/welcome.jsp',
                text: 'Bekleyin sayfa yükleniyor...',
...
//                    Ext.getCmp('panel2').enable();
			...
			tbar: [
                ...
                new Ext.app.SearchField({
                    ...
                    {//window.location = '../pages/login.jsp?go=1';
                tooltip:'Özet ekranýný tazele',
                text: 'Yenile',
                handler: function(){Ext.getCmp('welcome-panel').getUpdater().refresh();},
                scope: this,
                iconCls:'icon-docs'
            },'-',
            {
                tooltip:'Guvenli cikis',
                text: 'Oturumu kapat',
                handler: function(){window.location = 'pages/login.jsp?go=1';},
                scope: this,
...
Ext.extend(MainPanel, Ext.TabPanel, {
    ...
    onClick: function(e, target){
        if(target = e.getTarget('a:not(.exi)', 3)){
            ...
            if(cls){       
                ...
                this.loadClass(target.href, cls, member, target.text);
    ...
    loadClass : function(href, cls, member, text, ekle, sil, duzelt){
        ...
        if(tab){
            ...
            var autoLoad =
                ...
                scripts : true,     
                text: 'Bekleyin sayfa yükleniyor...',
                url: href + '?ekle=' + ekle + '&sil=' + sil + '&duzelt=' + duzelt
            };
            if(member){
...
Ext.onReady(function(){
    ...
    api.on('click', function(node, e){
         if(node.isLeaf()){
            ...
            //burdan yükleniyor ....
            mainPanel.loadClass(node.attributes.href, node.id, '', node.attributes.text, node.attributes.ekle, node.attributes.sil, node.attributes.duzelt);
         }
    });
    ...
    if(page){
        ...
        mainPanel.loadClass('output/' + cls + '.html', cls, ps.member, ps.text);
    }

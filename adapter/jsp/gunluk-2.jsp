...
if( action.trim().startsWith("gunluk") ){
	...
	String timeBaslangic = request.getParameter("bass");
    String timeBitis = request.getParameter("bits"); 	
    ...
    /*String x = "SELECT peron_tanim.tanim, peron_detay.aciklama, sum(peron_detay.fiyat) AS miktar, count(peron_detay.aciklama) as adet  "
            ...
            + "' and ( "
            + "(hour(now()) <= 13) and ( CAST(CONCAT(CAST(peron_detay.tarih1 AS DATE), ' ', peron_detay.saat1)  AS DATETIME) BETWEEN CAST(CONCAT('" + dateBaslangic + "', ' ', '" + timeBaslangic + "')  AS DATETIME) AND CAST(CONCAT('" + dateBitis + "', ' ', '" + timeBitis + "')  AS DATETIME) ) or "
            + "((hour(now()) > 13) and ( CAST(CONCAT(CAST(peron_detay.tarih1 AS DATE), ' ', peron_detay.saat1)  AS DATETIME) BETWEEN CAST(CONCAT('" + dateBaslangic + "', ' ', '" + timeBaslangic + "')  AS DATETIME) AND CAST(CONCAT('" + dateBitis + "', ' ', '" + timeBitis + "')  AS DATETIME) )  ) "
            + ") "

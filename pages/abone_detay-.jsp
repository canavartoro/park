...
<div id="abone_detay_info" class="info"></div>
...
function detay_yaz(gr, abone, alloff, yaz){	
 ...
 var alStore = new Ext.data.Store({
                    ...
                    proxy: new Ext.data.HttpProxy({
                        url: 'adapter/jsp/abone.jsp?action=tumalanlar'
                    }),
                    //totalProperty: 'totalCount',
                    remoteSort: true,
                    autoLoad: true
...
var comalan = new Ext.form.ComboBox({
                ...
                listeners:{
                    select:{fn:function(combo, value, index) {
							alert(Ext.getCmp('cmbalan').store.getAt(index).data['alan_id']);
							Ext.getCmp('cmbalan').store.filter('bolge_id', Ext.getCmp('cmbbolge').store.getAt(index).data['bolge_id']);
    ...
    var grid2 = new Ext.grid.GridPanel({
        ...
        cm: new Ext.grid.ColumnModel({
            ...
            columns: [
                ...
                {id:'bolge', header: "bolge", width: 135, dataIndex: 'bolge', hidden: false},
                {id:'bolge_id', header: "Bölge ID", width: 135, dataIndex: 'bolge_id', hidden: false}
            ]
        }),
        sm: sm,
        columnLines: true,
